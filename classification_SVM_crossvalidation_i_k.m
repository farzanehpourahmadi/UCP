...
for idx = find(yy==0)
   yy(idx) = -1; % The unit is off when yy2 is -1
end

%% Cross Validation (no. of folds=4)
for ii=1:4
    xx_v(:,1:d_n)=xx(:,(ii-1)*d_n+1:ii*d_n);
    yy_v(:,1:d_n)=yy(:,(ii-1)*d_n+1:ii*d_n);
    if ii>1
        xx_tr(:,1:(ii-1)*d_n)=xx(:,1:(ii-1)*d_n);
        yy_tr(:,1:(ii-1)*d_n)=yy(:,1:(ii-1)*d_n);
    end
    xx_tr(:,(ii-1)*d_n+1:n_n-d_n)=xx(:,ii*d_n+1:n_n);
    yy_tr(:,(ii-1)*d_n+1:n_n-d_n)=yy(:,ii*d_n+1:n_n);
